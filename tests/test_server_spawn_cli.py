...
class TestServerSpawnCli(unittest.TestCase):
    ...
    def test_spawn_web_session_sets_pi_env(self) -> None:
        mgr = self._mgr()
        with patch("codoxear.server._env_flag", return_value=False), patch(
            "codoxear.server._wait_or_raise", return_value=None
        ), patch("codoxear.server.subprocess.Popen", return_value=_DummyProc(6547)) as popen:
            res = mgr.spawn_web_session(cwd="/tmp", cli="pi")

        self.assertEqual(res.get("broker_pid"), 6547)
        self.assertEqual(res.get("cli"), "pi")
        env = popen.call_args.kwargs["env"]
        self.assertEqual(env.get("CODEX_WEB_CLI"), "pi")
